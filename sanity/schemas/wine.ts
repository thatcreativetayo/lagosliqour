...
export const wineSchema = {
  ...
  fields: [
    ...
    { name: "description", title: "Description", type: "text", validation: (Rule: ValidationRule) => Rule.required() },
    {
      name: "tastingNotes",
      title: "Tasting notes",
      type: "array",
    ...
    { name: "origin", title: "Origin", type: "string", validation: (Rule: ValidationRule) => Rule.required() },
    { name: "region", title: "Region", type: "string", description: "Backward-compatible display field. For spirits, use the same value as origin.", validation: (Rule: ValidationRule) => Rule.required() },
    { name: "abv", title: "ABV", type: "string", validation: (Rule: ValidationRule) => Rule.required() },
    { name: "alcoholContent", title: "Alcohol content", type: "string", description: "Backward-compatible alias for ABV.", validation: (Rule: ValidationRule) => Rule.required() },
    { name: "age", title: "Age / maturation", type: "string", description: "Example: 7 months, 14 months, 2+ years, unaged." },
    { name: "vintage", title: "Vintage / release year", type: "number", description: "Use the current catalog year for non-vintage spirits.", validation: (Rule: ValidationRule) => Rule.required().integer() },
    { name: "grapeVariety", title: "Variety / spirit type", type: "string", description: "Example: 100% Blue Weber Agave tequila.", validation: (Rule: ValidationRule) => Rule.required() },
    { name: "bottleSize", title: "Bottle size", type: "string", validation: (Rule: ValidationRule) => Rule.required() },
    {
      name: "bottleImage",
